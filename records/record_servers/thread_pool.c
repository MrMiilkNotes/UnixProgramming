...
thread_pool_t *pool_create(size_t sz_init, size_t sz_min, size_t sz_max,
  ...
  do {  // do...while(0)结构，失败需要清理
    if (sz_init < sz_min) {
      printf("error: sz_init < sz_min\n");
      break;
    } else if (sz_init > sz_max) {
      printf("error: sz_init > sz_max\n");
      break;
    }
    // -----------------------------------------------------
    // 创建线程池空间
    //      为线程池以及任务队列与工作线程队列分配空间
    //      初始化互斥锁与条件变量
    //      设置任务队列与工作队列的初始状态
    //      初始化管理者线程和默认线程
    // -----------------------------------------------------
...
int pool_add(thread_pool_t *t_pool, void (*handler)(void *arg), void *arg) {
  ...
  //  条件中的<是不应该成立的，否则任务数量超出任务队列容量
  //  这里是对于任务队列为满情况的处理，会等待非满的条件变量
  //  使用while: 来自APUE，(TODO)
...
void *thr_worker(void *arg) {
  ...
  while (1) {
    ...
    // 线程池进入销毁状态
    //  这部分必须放在上面的while后面，因为线程池可能在上面部分执行的时候销毁
    //  线程池的锁一放开，就有可能处于销毁状态
    if (t_pool_ptr->status == 0) {
      pthread_mutex_unlock(&(t_pool_ptr->lock));
      break;
